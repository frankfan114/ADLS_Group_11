...
module sim_tb_top;
  ...
  localparam [C_S_AXI_ADDR_WIDTH-1:0] AXI_BASE_B = 27'd16384;   // byte addr
  localparam [C_S_AXI_ADDR_WIDTH-1:0] AXI_BASE_C = 27'd32768;   // byte addr

  localparam [31:0] MATRIX_WORD_BASE_A = 32'd0;
  localparam [31:0] MATRIX_WORD_BASE_B = 32'd4096;
  localparam [31:0] MATRIX_WORD_BASE_C = 32'd8192;
  ...
  localparam integer CASE_DONE_TIMEOUT_POLLS = 2000000;
  ...
  // CASE0 uses baked DEiT-tiny CIFAR10 FC raw data:
  // A: input     [64 x 192] row-major int8
  // B: weight^T  [192 x 10] row-major int8
  // OUT in the .vh is quantized int8 golden sign-extended to 32-bit.
  // rsa_ws currently writes raw int32 accumulators, so pass/fail uses
  // A/B-derived raw matmul instead of direct OUT compare.
  localparam integer CASE0_JSON_BAKED = 1;
  localparam integer CASE0_COMPARE_OUTPUT_JSON = 0;
  localparam integer CASE0_M_DIM = 64;
  localparam integer CASE0_K_DIM = 192;
  localparam integer CASE0_N_DIM = 10;
  ...
  // Case index summary:
  //   0: 64x192 x 192x10, baked DEiT-tiny CIFAR10 FC JSON A/B
  ...
  `include "case0_json_data_deit_tiny_cifar10_fc_raw.vh"
  task get_case_dims;
...
// Controlling the bi-directional BUS
  ...
  begin : Logging
     ...
     bench_csv_fd = 0;
     $display("[TB_VERSION] DEIT_LAYER_FC_RAW_V1 | source=deit_layer/sim_tb_top.v | array=(%0d,%0d,%0d) | case0(M,K,N)=(%0d,%0d,%0d) | baked=%0d | cmp_json=%0d | t=%0t",
              MATRIX_MAX_M, MATRIX_MAX_K, MATRIX_MAX_N,
              CASE0_M_DIM, CASE0_K_DIM, CASE0_N_DIM,
              CASE0_JSON_BAKED, CASE0_COMPARE_OUTPUT_JSON, $time);
     if (CASE0_COMPARE_OUTPUT_JSON == 0)
       $display("[TB_NOTE] deit .vh OUT is quantized int8/sign-extended; pass/fail compares raw int32 accumulators recomputed from the same A/B.");
     sum_fetch_cycles   = 0;
     ...
     if (PRINT_BENCH_RESULT_CSV != 0) begin
       if ((MATRIX_MAX_M == 4) && (MATRIX_MAX_N == 4)) begin
         bench_csv_fd = $fopen("deit_layer_4_4_64x192x10_rawacc.csv", "w");
         if (bench_csv_fd == 0)
           $display("WARNING: could not open deit_layer_4_4_64x192x10_rawacc.csv for writing");
         else
           $display("Writing benchmark rows to deit_layer_4_4_64x192x10_rawacc.csv");
       end else if ((MATRIX_MAX_M == 8) && (MATRIX_MAX_N == 8)) begin
         bench_csv_fd = $fopen("deit_layer_8_8_64x192x10_rawacc.csv", "w");
         if (bench_csv_fd == 0)
           $display("WARNING: could not open deit_layer_8_8_64x192x10_rawacc.csv for writing");
         else
           $display("Writing benchmark rows to deit_layer_8_8_64x192x10_rawacc.csv");
       end else begin
         bench_csv_fd = $fopen("deit_layer_case0_64x192x10_rawacc.csv", "w");
         if (bench_csv_fd == 0)
           $display("WARNING: could not open deit_layer_case0_64x192x10_rawacc.csv for writing");
         else
           $display("Writing benchmark rows to deit_layer_case0_64x192x10_rawacc.csv");
       end
       if (bench_csv_fd != 0) begin
         $fdisplay(bench_csv_fd,
