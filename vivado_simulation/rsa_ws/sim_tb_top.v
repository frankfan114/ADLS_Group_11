...
module sim_tb_top;
  ...
  //***************************************************************************//
  // RSA/ADAPTNET Vivado multi-case quick-edit section
  ...
  // Case index summary:
  //   0 : 8x8  x 8x8  , dense auto config
  //   1 : 7x8  x 8x8  , manual sparse bypass mask
  //   2 : 3x8  x 8x3  , auto-selected 4x4
  //   3 : 3x8  x 8x16 , auto-selected 4x8
  //   4 : 16x8 x 8x3  , auto-selected 8x4
  //   5 : 2x8  x 8x2  , auto-selected 2x2
  task get_case_dims;
  ...
  begin
    case (tc_idx)
      ...
      1:  begin m_dim = 7;  k_dim = 8;  n_dim = 8;  end
      2:  begin m_dim = 3;  k_dim = 8;  n_dim = 3;  end
      3:  begin m_dim = 3;  k_dim = 8;  n_dim = 16; end
      4:  begin m_dim = 16; k_dim = 8;  n_dim = 3;  end
      5:  begin m_dim = 2;  k_dim = 8;  n_dim = 2;  end
  ...
  begin
    auto_cfg_en         = 1;
    manual_row_mask     = 8'hFF;
    manual_col_mask     = 8'hFF;
    expected_cfg_id     = 8'd0;
    expected_active_rows = 8'd8;
    expected_active_cols = 8'd8;
    expected_row_mask   = 8'hFF;
    expected_col_mask   = 8'hFF;
    ...
    case (tc_idx)
      ...
      1: begin
        auto_cfg_en          = 0;
        manual_row_mask      = 8'h55;
        manual_col_mask      = 8'h33;
        expected_cfg_id      = 8'h80;
        expected_active_rows = 8'd4;
        expected_active_cols = 8'd4;
        expected_row_mask    = 8'h55;
        expected_col_mask    = 8'h33;
      end
      ...
      2: begin
        ...
        expected_active_rows = 8'd4;
        expected_active_cols = 8'd4;
        expected_row_mask    = 8'h0F;
        expected_col_mask    = 8'h0F;
      end
      ...
      5: begin
        ...
        expected_active_rows = 8'd2;
        expected_active_cols = 8'd2;
        expected_row_mask    = 8'h03;
        expected_col_mask    = 8'h03;
      end
  ...
  begin
    case (tc_idx)
      0:  t = ((row_idx * 17) + (k_idx * 31)) % 256 - 128;      // dense auto
      1:  t = ((row_idx * 19) + (k_idx * 13) + 5) % 256 - 128;  // manual sparse
      2:  t = ((row_idx * 11) + (k_idx * 7) + 3) % 256 - 128;   // auto 4x4
      3:  t = ((row_idx * 23) + (k_idx * 9) + 1) % 256 - 128;   // auto 4x8
      4:  t = ((row_idx * 5) + (k_idx * 21) + 17) % 256 - 128;  // auto 8x4
      5:  t = ((row_idx * 29) + (k_idx * 15) + 7) % 256 - 128;  // auto 2x2
      default: t = 0;
    endcase
    gen_a_elem = t[7:0];
  end
  ...
  begin
    case (tc_idx)
      0:  t = ((k_idx * 13) + (col_idx * 29)) % 256 - 128;     // dense auto
      1:  t = ((k_idx * 7) + (col_idx * 23) + 11) % 256 - 128; // manual sparse
      2:  t = ((k_idx * 5) + (col_idx * 17) + 9) % 256 - 128;  // auto 4x4
      3:  t = ((k_idx * 3) + (col_idx * 27) + 15) % 256 - 128; // auto 4x8
      4:  t = ((k_idx * 31) + (col_idx * 9) + 13) % 256 - 128; // auto 8x4
      5:  t = ((k_idx * 21) + (col_idx * 19) + 1) % 256 - 128; // auto 2x2
      default: t = 0;
    endcase
    gen_b_elem = t[7:0];
  end
...
// Controlling the bi-directional BUS
  ...
  begin
    ...
    //   0x34 selected masks {16'h0, col_mask, row_mask}
    ...
    $display("CASE%0d selected cfg: cfg_id=%0d active_rows=%0d active_cols=%0d row_mask=%02h col_mask=%02h",
             ...
             selected_cfg_masks[7:0],
             selected_cfg_masks[15:8]);
    ...
    if ((selected_cfg_status[31:24] !== expected_cfg_id)          ||
        ...
        (selected_cfg_masks[7:0]    !== expected_row_mask)        ||
        (selected_cfg_masks[15:8]   !== expected_col_mask)) begin
  ...
  begin : Logging
     ...
     fork
        begin : run_matrix_multicase
           ...
           if (!axi_test_fail)
             $display("TEST PASSED: RSA/ADAPTNET MATRIX MULTI-CASE (%0d cases)", NUM_CASES);
           else
             $display("TEST FAILED: RSA/ADAPTNET MATRIX MULTI-CASE");
        ...
        begin : test_timeout
           ...
           if (!init_calib_complete)
             ...
             $display("TEST FAILED: RSA/ADAPTNET MATRIX MULTI-CASE TIMEOUT");
