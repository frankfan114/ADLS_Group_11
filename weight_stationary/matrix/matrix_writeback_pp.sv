...
module matrix_writeback_pp #(
    ...
    // Advance either on a real handshake or on an internal skip of zero word.
    assign wb_fire   = (state == WB_RUN) && (skip_this_word || wb_ready);
    ...
    assign wb_valid       = (state == WB_RUN) && !skip_this_word;
