# WS-PP Simulation Test Cases

The cocotb regression in this directory is `matrix_axi_wrapper_tb.py`, which verifies `matrix_top_wrapper`.

By default, `ws_pp/rtl/matrix_top_wrapper.v` builds the validated safe path and only enables the experimental PP tiled path when `USE_PP_TILED` is defined.
