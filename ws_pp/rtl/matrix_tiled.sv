...
`ifndef SYNTHESIS
    always @(posedge clk) begin
        if (rst_n && start && (num_tile_m > ACC_TILE_SLOTS)) begin
            $fatal(1,
                   "ws_pp matrix_tiled ACC_TILE_SLOTS overflow: have %0d slots, need %0d M-tiles",
                   ACC_TILE_SLOTS,
                   num_tile_m);
        end
    end
`endif
    ...
    // Split the accumulator store into narrow element banks so Vivado can map
    // it as supported memories instead of one ultra-wide RAM per M tile.
        ...
        if (!rst_n) begin
            ...
            if ((c_state == CS_ACCUM) && (tile_m_idx < ACC_TILE_SLOTS)) begin
                for (int j = 0; j < TOTAL_ELEMS_TILE; j++) begin
                    if (first_tk) begin
                        c_tile_spad[j][tile_m_idx] <= core_partial_flat_r[j*ACC_W +: ACC_W];
                    end else begin
                        c_tile_spad[j][tile_m_idx] <=
                            c_tile_spad[j][tile_m_idx] + core_partial_flat_r[j*ACC_W +: ACC_W];
                    end
                end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            wb_partial_flat <= '0;
        end else if (wb_issue_fire) begin
            wb_partial_flat <= '0;
            if (wb_issue_m_idx < ACC_TILE_SLOTS) begin
                for (int j = 0; j < TOTAL_ELEMS_TILE; j++) begin
                    wb_partial_flat[j*ACC_W +: ACC_W] <= c_tile_spad[j][wb_issue_m_idx];
                end
            end
        end
    end
