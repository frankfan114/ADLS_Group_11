...
async def check_status(dut, *, busy, done, context):
    status = await bus_read(dut, 0x20)
    got_busy = status & 0x1
    got_done = (status >> 1) & 0x1
    if got_busy != busy or got_done != done:
        raise AssertionError(
            f"{context}: expected status busy={busy} done={done}, got status=0x{status:08x}"
        )


async def check_programmed_registers(dut, *, baseA_word, baseB_word, baseC_word, M, K, N):
    expected_regs = [
        (0x00, baseA_word, "base_addr_A"),
        (0x04, baseB_word, "base_addr_B"),
        (0x08, baseC_word, "base_addr_C"),
        (0x0C, M, "M"),
        (0x10, K, "K"),
        (0x14, N, "N"),
    ]
    for addr, exp, name in expected_regs:
        got = await bus_read(dut, addr)
        if got != exp:
            raise AssertionError(f"register {name} readback mismatch: exp=0x{exp:08x} got=0x{got:08x}")
...
async def run_one_case(
    ...
    *,
    ...
    await check_programmed_registers(
        dut,
        baseA_word=baseA_word,
        baseB_word=baseB_word,
        baseC_word=baseC_word,
        M=M,
        K=K,
        N=N,
    )
    ...
    await check_status(dut, busy=0, done=1, context=f"{case_name} after completion")
...
async def test_matrix_top_wrapper_axi(dut):
    ...
    for case in isolated_cases:
        await reset_dut(dut)
        await check_status(dut, busy=0, done=0, context=f"{case['case_name']} after reset")
    ...
    await reset_dut(dut)
    await check_status(dut, busy=0, done=0, context="repeat sequence after reset")
