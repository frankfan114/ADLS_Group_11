...
module matrix_fetch #(
    ...
    // Derived parameters
    // ============================================================
    localparam int ELEMS_PER_SRAM    = SRAM_WIDTH / DATA_WIDTH; // 4
    localparam int ELEMS_SHIFT       = $clog2(ELEMS_PER_SRAM);
    localparam int MAX_WORDS_PER_ROW = MAX_COL_NUM / ELEMS_PER_SRAM;
    localparam int BUFFER_BITS       = MAX_COL_NUM * DATA_WIDTH;

    logic [$clog2(MAX_WORDS_PER_ROW+1)-1:0] words_per_row;
    always_comb begin
        words_per_row = (col_num + (ELEMS_PER_SRAM-1)) >> ELEMS_SHIFT;
    end
    ...
    typedef enum logic [2:0] {
        S_IDLE,
        S_REQ,
        S_WAIT,     // rd_valid=1, wait rd_ready
        S_GAP,      // rd_valid=0, explicit transaction gap
        S_ROW_OUT,
        S_DONE
    } state_t;
    ...
    // ============================================================
    integer b;
    logic [$clog2(MAX_COL_NUM+1)-1:0] elem_base;
    logic [$clog2(MAX_COL_NUM+1)-1:0] rem_elems;
    logic [2:0]                       valid_elems;

    always_comb begin
        elem_base = (word_idx << ELEMS_SHIFT);

        if (col_num > elem_base)
            rem_elems = col_num - elem_base;
        else
            rem_elems = '0;

        if (rem_elems >= ELEMS_PER_SRAM)
            valid_elems = ELEMS_PER_SRAM;
        else
            valid_elems = rem_elems;
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                // fire: rd_valid && rd_ready
                S_WAIT: begin
                    if (rd_ready) begin
                        for (b = 0; b < ELEMS_PER_SRAM; b++) begin
                            if (b < valid_elems)
                                buffer[((elem_base + b)*DATA_WIDTH) +: DATA_WIDTH]
